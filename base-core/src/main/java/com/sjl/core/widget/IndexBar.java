...
public class IndexBar extends LinearLayout {
    ...
    private void init(AttributeSet attrs) {
        ...
        float indexTextSize = ta.getDimension(R.styleable.IndexBar_indexTextSize, ViewUtils.sp2px(getContext(), 12));
    ...
    private void handle(MotionEvent event) {
        ...
        int position = INDEXES.length * y / height;//计算按下的字母的索引的位置
        if (position < 0) {
            position = 0;
