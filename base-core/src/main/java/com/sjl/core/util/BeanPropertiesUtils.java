 ...
 * @author Kelly
 * @version 1.0.0
 * @filename BeanPropertiesUtils.java
...
public class BeanPropertiesUtils {
     ...
     */
    @SuppressWarnings("unchecked")
    public static void copyPropertiesInclude(Object from, Object to, String[] excludeArray) throws Exception {
        List<String> includesList = null;
        if (excludeArray != null && excludeArray.length > 0) {
            includesList = Arrays.asList(excludeArray);    //构造列表对象
        } else {
            return;
        }
        Method[] fromMethods = from.getClass().getDeclaredMethods();
